...
export function KanbanBoard() {
    ...
    const { data: sellersData, isLoading: isLoadingSellers } = useQuery({
        queryKey: ["sellers", brokerFilter, activeFunnelId],
        queryFn: async () => {
            const res = await api.get("/sellers", { params: { brokerId: brokerFilter } });
            return res.data;
        },
    ...
    const { data: propertiesData, isLoading: isLoadingProperties } = useQuery({
        ...
        queryFn: async () => {
            const res = await api.get("/crm-properties", { params: { brokerId: brokerFilter } });
            return res.data;
        },
    ...
    const getStageId = (item: Seller | CrmProperty) => {
        if (crmMode === "CUSTOM") return item.customStageId || "uncategorized";
    ...
    const getColumns = () => {
        if (crmMode === "CUSTOM") {
            ...
            return activeFunnel.stages.map(stage => ({
                id: stage.id, title: stage.name, color: stage.color, variant: 'default'
            }));
        }
    ...
    const sellersGrouped = sellersFiltered.reduce((acc: Record<string, Seller[]>, seller: Seller) => {
    ...
    const propertiesGrouped = propertiesFiltered.reduce((acc: Record<string, CrmProperty[]>, prop: CrmProperty) => {
    ...
    return (
        <div className="h-full flex flex-col space-y-4">
            <CreateSellerForm
                ...
                onOpenChange={(v) => { setIsSellerFormOpen(v); if (!v) setSelectedSeller(null); }}
                onSuccess={() => { queryClient.invalidateQueries({ queryKey: ["sellers"] }); setIsSellerFormOpen(false); }}
                activeFunnelId={activeFunnelId}
                initialData={selectedSeller as any}
            />
            <CreatePropertyForm
                open={isPropertyFormOpen}
                onOpenChange={(v) => { setIsPropertyFormOpen(v); if (!v) setSelectedProperty(null); }}
                sellerId={selectedSellerId}
                initialData={selectedProperty as any}
            />
            {propertyConfigToClose && (
                <CloseDealDialog open={isCloseDealOpen} onOpenChange={setIsCloseDealOpen} propertyId={propertyConfigToClose.id} onSuccess={handleCloseDealSuccess} />
            )}

            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div className="flex items-center gap-4">
                    {!isCustom && (
                        <Tabs value={activeTab} onValueChange={(val) => setActiveTab(val as "sellers" | "properties")} className="w-auto">
                            <TabsList>
                        ...
                        <div className="flex items-center gap-2">
                            {isLoadingFunnels ? (
                                ...
                                <Select value={activeFunnelId || ""} onValueChange={setActiveFunnelId}>
                                    <SelectTrigger className="w-[200px]"><SelectValue placeholder="Выберите воронку" /></SelectTrigger>
                                    <SelectContent>
                                        {customFunnels?.map(f => (<SelectItem key={f.id} value={f.id}>{f.name}</SelectItem>))}
                                    </SelectContent>
                                </Select>
                            )}
                ...
                {!isCustom && (
                    <div className="flex gap-2 items-center flex-wrap relative z-10">
                        <div className="flex bg-muted rounded-lg p-1">
                            <Button variant={viewMode === "kanban" ? "secondary" : "ghost"} size="sm" className="h-7 px-3" onClick={() => setViewMode("kanban")}>
                                <LayoutGrid className="h-4 w-4 mr-2" />Канбан
                            </Button>
                            <Button variant={viewMode === "list" ? "secondary" : "ghost"} size="sm" className="h-7 px-3" onClick={() => setViewMode("list")}>
                                <List className="h-4 w-4 mr-2" />Список
                            </Button>
                        </div>
                    </div>
                )}
                {userRole === "ADMIN" && <BrokerFilter value={brokerFilter} onChange={setBrokerFilter} className="w-[180px]" />}
                {userRole === "BROKER" && userId && <FormLinksButton userId={userId} />}
                ...
                {(activeTab === "sellers" || isCustom) && (
                    <Button size="sm" onClick={() => setIsSellerFormOpen(true)}>
                        <Plus className="mr-2 h-4 w-4" />{isCustom ? "Новая сделка" : "Новый продавец"}
                    </Button>
                )}
            </div>
            ...
            <div className="flex-1 overflow-auto bg-muted/10 p-4 rounded-lg border">
                {isCustom && !activeFunnel ? (
                    <div className="flex flex-col items-center justify-center h-full text-muted-foreground gap-4">
                        ...
                        <Link href="/dashboard/settings/funnels"><Button>Создать воронку</Button></Link>
                    </div>
                ) : (
                    <>
                        {(activeTab === "sellers" || isCustom) ? (
                            isLoadingSellers ? (
                                ...
                                <SellersListView onEdit={handleEditSeller} activeFunnelId={activeFunnelId} />
                            ) : (
                                <DndBoard type="sellers" columns={columns as any} items={sellersGrouped} onAddProperty={handleAddProperty} isCustom={isCustom}
                                    onDragEnd={(id, stage) => { updateSellerStageMutation.mutate({ id, stage, isCustom }); }} />
                            )
                        ) : (
                            isLoadingProperties ? (
                                <div className="flex items-center justify-center h-full text-muted-foreground">Загрузка объектов...</div>
                            ) : viewMode === "list" ? (
                                <PropertiesListView onEdit={(prop) => { setSelectedProperty(prop); setIsPropertyFormOpen(true); }} activeFunnelId={activeFunnelId} />
                            ) : (
                                <DndBoard type="properties" columns={columns as any} items={propertiesGrouped} onDragEnd={handlePropertyDragEnd} isCustom={isCustom}
                                    onEditProperty={(prop: CrmProperty) => { setSelectedProperty(prop); setIsPropertyFormOpen(true); }} />
                            )
                        )}
                    </>
                )}
